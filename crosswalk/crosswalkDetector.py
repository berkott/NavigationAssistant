...
def getLowestPoint(point1, point2):
    if point1[1] < point2[1]:
        return point2
    return point1


def getBottomPoints(box):
    closest_to_0 = 0
    min_distance = 1000
    for i in range(1, 4):
        current_distance = distance(box[0], box[i])
    ...
    for i in range(1, 4):
        if not closest_to_0 == i:
            otherPoints.append(i)

    isRight = False

    if (box[0][0] + box[closest_to_0][0]) > (box[otherPoints[0]][0] + box[otherPoints[1]][0]):
        isRight = True

    if isRight:
        right_point = getLowestPoint(box[0], box[closest_to_0])
        left_point = getLowestPoint(box[otherPoints[0]], box[otherPoints[1]])
    else:
        left_point = getLowestPoint(box[0], box[closest_to_0])
        right_point = getLowestPoint(box[otherPoints[0]], box[otherPoints[1]])
...
def process(im):
    ...
    for i in contours:
        ...
        cv2.imshow("poly-rect", cv2.drawContours(filtered, [box], 0, (0, 0, 255), 3))
        # cv2.imshow("rect", cv2.rectangle(filtered, (bx, by), (bx + bw, by + bh), (0, 255, 0), 3))

        bot_r, bot_l = getBottomPoints(box)

        # print(bot_r)

        bot_dist = distance(bot_r, bot_l)

        # print(bot_dist)

        if bot_dist > bw_width:
            cv2.line(im, (bot_r[0], bot_r[1]), (bot_l[0], bot_l[1]), (0, 255, 0), 2)  # draw the a contour line
            bxRight.append(bot_r[0])  # right line
            byRight.append(bot_r[1])  # right line
            bxLeft.append(bot_l[0])  # left line
            byLeft.append(bot_l[1])  # left line
            bxbyLeftArray.append(bot_r)  # x,y for the left line
            bxbyRightArray.append(bot_l)  # x,y for the left line
            cv2.circle(im, (int(bot_l[0]), int(bot_l[1])), 5, (0, 250, 250), 2)  # circles -> left line
            cv2.circle(im, (int(bot_r[0]), int(bot_r[1])), 5, (250, 250, 0), 2)  # circles -> right line
