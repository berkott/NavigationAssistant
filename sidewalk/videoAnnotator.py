...
# SOME OF THE RHO DATA IS NEGATIVE AND SOME IS POSITIVE SO EVERYTHING NEEDS TO BE POSITIVE WITH PI
# SUBTRACTED
...
def getLinesFromBoundary(left, right):
    ...
    leftLines = cv2.HoughLines(boundaryImageLeft, 1, np.pi / 180, 30)
    rightLines = cv2.HoughLines(boundaryImageRight, 1, np.pi / 180, 30)
    ...
    leftLinesOG = cv2.HoughLines(inverseWarpImage(boundaryImageLeft), 1, np.pi / 180, 30)
    rightLinesOG = cv2.HoughLines(inverseWarpImage(boundaryImageRight), 1, np.pi / 180, 30)
...
lineFromMouse = [[0, 0], [0, 0]]


def on_mouse(event, x, y, flags, params):
    if event == cv2.EVENT_LBUTTONDOWN:
        initialPoint = [x, y]
        lineFromMouse[0] = initialPoint
        # print(initialPoint)

    elif event == cv2.EVENT_LBUTTONUP:
        finalPoint = [x, y]
        lineFromMouse[1] = finalPoint
        # print(finalPoint)


def getLineFromPoints(lineFromMouse, linesOG, side):
    # Linesog is left, right
    if (lineFromMouse[1][0] - lineFromMouse[0][0]) == 0:
        theta = 0
        rho = lineFromMouse[0][0]
    else:
        m = (lineFromMouse[1][1] - lineFromMouse[0][1]) / (lineFromMouse[1][0] - lineFromMouse[0][0])
        b = lineFromMouse[0][1] - (m * lineFromMouse[0][0])
    ...
    if side == "right":
        linesOG[1][0] = [rho, theta]
    else:
        linesOG[0][0] = [rho, theta]
...
while cap.isOpened():
    ...
    if ret:
        ...
        cv2.setMouseCallback('linedOriginal', on_mouse, 0)
        ...
        while True:
            ...
            key = cv2.waitKey()
            if key == ord('d'):
                if (lineFromMouse is not None) and (lineFromMouse[0] != lineFromMouse[1]):
                    # print("Left line:", lineFromMouse)
                    linesOG = getLineFromPoints(lineFromMouse, linesOG, "left")
                    linedOriginal = getLinedImage(originalImage.copy(), linesOG)
                    cv2.imshow("linedOriginal", linedOriginal)

            if key == ord('f'):
                if (lineFromMouse is not None) and (lineFromMouse[0] != lineFromMouse[1]):
                    # print("Right line:", lineFromMouse)
                    linesOG = getLineFromPoints(lineFromMouse, linesOG, "right")
                    linedOriginal = getLinedImage(originalImage.copy(), linesOG)
                    cv2.imshow("linedOriginal", linedOriginal)
            # print(linesOG)
